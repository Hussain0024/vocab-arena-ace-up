...
import { Book, Trophy, User, Star, Settings } from 'lucide-react';
...
const Navigation = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 px-4 py-3">
      ...
      <div className="flex justify-center mt-4 bg-gray-100 rounded-xl p-1 max-w-sm mx-auto">
        {navItems.map((item) => (
          <button
            key={item.id}
            ...
            className={`flex-1 flex flex-col items-center py-2 px-3 rounded-lg transition-all duration-200 ${
              activeTab === item.id
                ? 'bg-white shadow-sm text-game-purple'
